...
import { AlertTriangle, ExternalLink, Loader2, Receipt, RotateCw, UtensilsCrossed } from "lucide-react"
...
import { itemTotalOf, kGross, kItemTotal, kNet } from "@/lib/gross-net"
...
const Line = ({ label, value, strong }: { label: string; value: React.ReactNode; strong?: boolean }): React.JSX.Element => (
...
const Field = ({ label, value }: { label: string; value: React.ReactNode }): React.JSX.Element => (
...
export function DrillDownDialog({ request, onClose, restaurantId, outletId, timezone, currencySymbol }: Props): React.JSX.Element {
    ...
    const money = (n: number | null | undefined): string => formatMoney(n, currencySymbol)
    const when = (v: string | null | undefined): string => (v ? formatFullDateTime(v, timezone) : "—")
    ...
    useEffect(() => {
        ...
        return () => { active = false }
    }, [request, restaurantId, outletId, attempt])
                ...
                {!loading && bill && (
                    <div className="space-y-4">
                                ...
                                {bill.items.map((item, i) => (
                                    <div key={`${item.name}-${String(i)}`} className="flex items-baseline justify-between gap-4 border-b px-3 py-1.5 text-sm last:border-b-0">
                                        <span className="min-w-0">
                                            <span className="text-muted-foreground">{item.quantity} ×</span> {(item as { nc?: unknown }).nc === true ? `${item.name} (NC)` : item.name}
                        ...
                        <div className="rounded-md border bg-muted/30 px-3 py-2 text-sm">
                            <Line label={kItemTotal} value={money((itemTotalOf(bill as unknown as Record<string, unknown>) as number | null | undefined) ?? bill.items_subtotal)} />
                            ...
                            <Line label={kNet} value={money(bill.taxable_base)} />
                            {bill.service_charge ? (
                            ...
                            {(() => {
                                const ro = roundOffOf(bill)
                                return ro !== null ? <Line label="Round off" value={formatRoundOff(ro, money)} /> : null
                            })()}
                            <Separator className="my-1.5" />
                            <Line label={kGross} value={money(bill.grand_total)} strong />
                            {bill.refunded && (
                ...
                {!loading && kot && (
                    <div className="space-y-4">
                        <div className="flex flex-wrap gap-2">
                            <StatusChip
                                dense
                                label={String(kot.status)}
                                status={/cancel|void/i.test(String(kot.status)) ? "danger" : "neutral"}
                            />
                        </div>
                        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                        ...
                        <div>
                            ...
                            <div className="rounded-md border">
                                ...
                                {kot.items.length === 0 && (
                                    <div className="px-3 py-4 text-center text-sm text-muted-foreground">This ticket carries no item lines.</div>
                                )}
                            </div>
                            ...
                            <div className="rounded-md border">
                                ...
                                {kot.trail.length === 0 && (
                                    <div className="px-3 py-4 text-center text-sm text-muted-foreground">
                                        No audit entry names this order.
                                    </div>
                                )}
                            </div>
                        </div>
