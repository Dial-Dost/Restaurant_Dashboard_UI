...
/**
 * THE QUEUE PRE-ORDER MENU editor (modules.dart `_QueueMenuDialog`), opened
 * straight from the menu toolbar's "Queue pre-order menu" button.
 *
 ...
 * wants. This dialog is the second, narrower view of the same menu.
 *
...
import { fetchQueueMenuConfig } from "@/lib/api/menu";
...
/**
 * The toolbar entry point: loads /queue-menu-config itself (Flutter's dialog
 * shows its own loading state), then hands the payload to the editor.
 * `onClose(changed)` — changed = anything was saved, so the menu reloads.
 */
export function QueuePreorderMenuDialog({ restaurantId, onClose }: {
    restaurantId: string;
    onClose: (changed: boolean) => void;
}): React.JSX.Element {
    const config = useCachedFetch<QueueMenuConfigPayload>(
        `menu:queue-config:${restaurantId}`,
        fetchQueueMenuConfig,
        { enabled: !!restaurantId },
    );

    if (config.loading || config.error != null || config.data == null) {
        return (
            <Dialog open onOpenChange={(o) => { if (!o) { onClose(false); } }}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader className="text-left">
                        <div className="micro-label mb-1.5">Walk-in queue</div>
                        <DialogTitle>Queue pre-order menu</DialogTitle>
                        <DialogDescription className="sr-only">Queue pre-order menu</DialogDescription>
                    </DialogHeader>
                    {config.loading ? (
                        <SkeletonRows rows={4} />
                    ) : (
                        <LoadErrorState
                            whatFailed="Couldn't load the queue menu settings."
                            error={config.error}
    ...
    return <QueuePreorderEditor restaurantId={restaurantId} payload={config.data} onClose={onClose} />;
}

function QueuePreorderEditor({ restaurantId, payload, onClose }: {
    restaurantId: string;
    ...
    const toggleList = (field: "items" | "categories", value: string): void => {
        setDraft((d) => {
    ...
    const moveCategory = (category: string, delta: number): void => {
        setDraft((d) => {
    ...
    const reset = async (): Promise<void> => {
        const ok = await confirm({
            title: "Show the whole menu again",
            body: "Queuing guests will be able to pre-order anything on the menu, and your custom wording is removed.",
        ...
        <Dialog open onOpenChange={(o) => { if (!o) { onClose(false); } }}>
            <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
                <DialogHeader className="text-left">
                    ...
                    <DialogTitle>Queue pre-order menu</DialogTitle>
                    <DialogDescription>
                        What a guest waiting in the queue can order before they sit down. Leaving a dish off
                        here does <strong>not</strong> take it off the dine-in menu.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="grid gap-6 md:grid-cols-2">
                    <div className="space-y-6">
                            ...
                            <div className="grid gap-2">
                                {MODES.map((m) => (
                                    <button
                                        ...
                                        className={`rounded-[10px] border px-3 py-2 text-left text-sm transition-colors duration-fast ${draft.mode === m.value ? "border-accent-base/55 bg-accent-base/12" : "border-border bg-inset hover:bg-foreground/[0.04]"}`}
                                    >
                        ...
                        {draft.mode !== "all" && (
                            <section className="space-y-3">
                                    ...
                                    {allCategories.map((c) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`rounded-full border px-3 py-1 text-xs transition-colors duration-fast ${on ? "border-accent-base/55 bg-accent-base/12 font-medium text-accent-foreground" : "border-border bg-inset hover:bg-foreground/[0.04]"}`}
                                            >
                                                {c}
                                            </button>
                                        );
                                ...
                                {/* [web-extra] search over the tick list — Flutter's 240px list has none. */}
                                ...
                                <div className="max-h-64 space-y-1 overflow-y-auto rounded-[10px] border border-border p-2">
                                    ...
                                    {shown.map((it) => {
                                        ...
                                        return (
                                            <label
                                                key={it.id}
                                                className="flex cursor-pointer items-center gap-3 rounded px-2 py-1.5 text-sm hover:bg-foreground/[0.04]"
                                            >
                                                <Checkbox
                                                    checked={ticked}
                            ...
                            <section className="space-y-2">
                                ...
                                <div className="space-y-1">
                                    {orderedCategories.map((c, i) => (
                                        <div key={c} className="flex items-center gap-2 rounded-[10px] border border-border px-3 py-1.5 text-sm">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="rounded-lg border border-border bg-inset p-4">
                            ...
                            {previewCount === 0 ? (
                                <p className="mt-4 rounded-[10px] border border-dashed border-input p-4 text-center text-xs text-muted-foreground">
                                ...
                                <>
                                    <div className="mt-3 flex flex-wrap gap-1.5">
                                        {preview.map((g, i) => (
                                            <span key={g.category} className={`rounded-full px-2.5 py-1 text-[11px] ${i === 0 ? "bg-accent-base text-accent-on" : "bg-card text-muted-foreground"}`}>
                                                {g.category}
                                            </span>
                                        ))}
                                    </div>
                                    <div className="mt-3 space-y-1.5">
                                        {(preview[0]?.items ?? []).map((it) => (
                                            <div key={it.id} className="flex items-center justify-between gap-3 rounded-[10px] bg-card px-3 py-2 text-sm">
                                                <span className="truncate">{it.name}</span>
                                                {draft.show_prices && <span className="text-xs tabular-nums text-muted-foreground">₹{it.price}</span>}
                                            </div>
                                        ))}
                                    </div>
                                    <p className="mt-2 text-[11px] text-muted-foreground">
                                        Showing the first tab · {previewCount} of {payload.items.length} dishes offered
                                    </p>
                                </>
                            )}
                        </div>
                ...
                <DialogFooter className="gap-2 sm:justify-between">
                    ...
                    <div className="flex gap-2">
                        <Button type="button" variant="outline" onClick={() => { onClose(false); }} disabled={busy}>Cancel</Button>
